...
class CA_NET(nn.Module):
    ...
    def __init__(self):
        ...
        self.t_dim = cfg.SPEECH.DIMENSION
        self.ef_dim = cfg.GAN.EMBEDDING_DIM
        self.fc = nn.Linear(self.t_dim, self.ef_dim * 4, bias=True)
        self.relu = GLU()

    def encode(self, speech_embedding):
        x = self.relu(self.fc(speech_embedding))
        mu = x[:, :self.ef_dim]
    ...
    def forward(self, speech_embedding):
        mu, logvar = self.encode(speech_embedding)
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ...
        self.upsample = upBlock(ngf*2, ngf // 2)
    ...
    def forward(self, h_code, c_code):
        ...
        out_code = torch.cat((out_code,h_code),1)
...
class G_NET(nn.Module):
    ...
    def forward(self, z_code, speech_embedding=None):
        if cfg.GAN.B_CONDITION and speech_embedding is not None:
            c_code, mu, logvar = self.ca_net(speech_embedding)
        else:
